...
```bash
npm install @punch/ai
```

TypeBox exports are re-exported from `@punch/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@punch/ai';
import { builtinModels } from '@punch/ai/providers/all';
...
import { openaiProvider } from '@punch/ai/providers/openai';
import { openrouterProvider } from '@punch/ai/providers/openrouter';
import { amazonBedrockProvider } from '@punch/ai/providers/amazon-bedrock';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@punch/ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@punch/ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@punch/ai';
...
```typescript
import { validateToolCall, type Tool } from '@punch/ai';
...
```typescript
import { builtinImagesModels } from '@punch/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@punch/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@punch/ai';
import { openAICompletionsApi } from '@punch/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@punch/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@punch/ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@punch/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@punch/ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@punch/ai';
import { anthropicProvider } from '@punch/ai/providers/anthropic';
import { openaiProvider } from '@punch/ai/providers/openai';
import { googleProvider } from '@punch/ai/providers/google';
...
- `@punch/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@punch/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@punch/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@punch/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@punch/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@punch/ai/api/bedrock-converse-stream.lazy';
...
```bash
npx @punch/ai login              # interactive provider selection
npx @punch/ai login anthropic    # login to specific provider
npx @punch/ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@punch/ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@punch/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@punch/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@punch/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@punch/ai`.
