	...
	return {
		...
		async authorize(context: Context): Promise<void> {
			...
			if (
				...
				!(await load()).some(
					(entry) =>
						entry.principal.userId === principal.userId &&
						entry.principal.workspaceId === principal.workspaceId &&
						principal.permissions.every((permission) => entry.principal.permissions.includes(permission)),
				)
			)
