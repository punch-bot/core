...
- `@punch-bot/chord` must be externalized so a plugin uses the host's one runtime and branding symbols.
...
If Node-only APIs are exported, they should use a separate package export such as `@punch-bot/chord/node` or `@punch-bot/chord/bundler`; importing the main runtime must not load Node-only modules.
