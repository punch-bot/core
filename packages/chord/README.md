...
`@punch-bot/chord`. Import public types and general runtime APIs from the
package root. Context constants and functions live in
`@punch-bot/chord/context` because their generic names should not pollute
the root API.
...
Import the standalone delta primitive from `@punch-bot/chord/delta`:

```ts
import { apply, track } from "@punch-bot/chord/delta";
...
`@punch-bot/chord/bundler` uses esbuild to turn ESM or TypeScript application
...
```ts
import { bundleFacetPackage } from "@punch-bot/chord/bundler";
...
```ts
import { createFacetBundleLoader } from "@punch-bot/chord/node";
