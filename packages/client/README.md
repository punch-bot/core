...
```ts
import { PiClient, type ByteTransportFactory } from "@punch/client";
...
import { createUnixTransportFactory } from "@punch/client/unix";
...
The `@punch/client` root remains transport- and runtime-neutral. Importing the Node-compatible transport requires the explicit `@punch/client/unix` subpath.
