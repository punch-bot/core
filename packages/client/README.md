...
```ts
import { PiClient, type ByteTransportFactory } from "@punch-bot/client";
...
The `@punch-bot/client` root remains transport- and runtime-neutral. Importing the Node-compatible transport requires the explicit `@punch-bot/client/unix` subpath.
