...
import { readFacetBundleManifest } from "@punch-bot/chord/node";
	...
	test("builds conventional plugin entries into the server-owned plugin cache", async () => {
		...
		await writeFile(
			...
			`${JSON.stringify({
				...
				version: "1.0.0",
				peerDependencies: {
					"@punch-bot/chord": "^0.84.4",
					"@punch-bot/cli": "^0.84.4",
				},
			})}\n`,
		);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@punch-bot/chord"; import { SlashCommands } from "@punch-bot/cli/experimental/plugin"; export default defineFacet({ id: "built-a", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@punch-bot/chord"; import { SlashCommands } from "@punch-bot/cli/experimental/plugin"; export default defineFacet({ id: "built-b", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		await Promise.all([
			writeFile(
				...
				`${JSON.stringify({
					...
					version: "1.0.0",
					peerDependencies: { "@punch-bot/chord": "^0.84.4" },
				})}\n`,
			),
			writeFile(
				join(secondPackagePath, "src", "tui.ts"),
				'import { defineFacet } from "@punch-bot/chord"; export default defineFacet({ id: "second-built", setup() {} });\n',
			),
		]);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@punch-bot/chord"; import { SlashCommands } from "@punch-bot/cli/experimental/plugin"; export default defineFacet({ id: "built-c", setup(env) { env.use(SlashCommands); } });\n',
		);
