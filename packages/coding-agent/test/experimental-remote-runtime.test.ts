...
import { type Context, createFacetHost, defineFacet, defineService } from "@punch-bot/chord";
import { BACKGROUND_CONTEXT } from "@punch-bot/chord/context";
import { Client, ServerError as ClientServerError } from "@punch-bot/client";
import { createUnixTransportFactory } from "@punch-bot/client/unix";
	...
	test("loads conventional Session facets from multiple configured plugin packages", async () => {
		...
		await Promise.all([
			...
			writeFile(
				...
				'import { defineFacet, defineService } from "@punch-bot/chord"; const Service = defineService("test.second-plugin"); export default defineFacet({ id: "second-session-plugin", setup(env) { env.provide(Service, { async read() { return "second"; } }); } });\n',
			),
		]);
		const runtime = await startServer({ ...sessionWorkerModel, directory });
		servers.add(runtime);
