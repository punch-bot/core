...
function createFixture(t, { importServer = false, declareServer = false } = {}) {
	...
	const packages = [codingAgentName, "@punch-bot/chord", ...devPackages].map((name) => ({
		name,
	...
	for (const pkg of packages) {
		...
		const manifest = {
			...
			...(isAgent ? {
				...
				dependencies: {
					"@punch-bot/chord": "1.0.0",
					...(declareServer ? { "@punch-bot/server": "1.0.0" } : {}),
				},
...
${importServer ? 'import "@punch-bot/server";' : ""}
import { marker } from "@punch-bot/chord";
