...
const developmentPackages = new Set(["client", "protocol", "server"].map((name) => `@punch-bot/${name}`));
...
for (const name of ["client", "protocol", "server"]) {
  assert.throws(() => import.meta.resolve("@punch-bot/" + name), /Cannot find|cannot find/, name + " must not be installed");
}
