...
test("synchronizes private dependencies without touching registry aliases, generated manifests, or published lockstep", async () => {
	...
	try {
		...
		await writeManifest(root, "packages/evals", {
			...
			dependencies: {
				"@punch/cli": "^1.0.0",
				"@mariozechner/pi-ai": "npm:@punch/ai@1.0.0",
			},
		});
		...
		assert.equal(evalsManifest.dependencies["@punch/cli"], "^2.0.0");
		assert.equal(evalsManifest.dependencies["@mariozechner/pi-ai"], "npm:@punch/ai@1.0.0");
		const generatedManifest = await readManifest(root, "packages/coding-agent/install-lock");
		assert.equal(generatedManifest.dependencies["@punch/cli"], "^1.0.0");
